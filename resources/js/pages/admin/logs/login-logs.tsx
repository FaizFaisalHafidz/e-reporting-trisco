...
import { Activity, AlertTriangle, Calendar, CheckCircle, Download, Eye, Filter, LogOut, Search, Shield, Users } from 'lucide-react';
...
export default function LoginLogs({ logs, users, stats, filters }: Props) {
    ...
    const applyFilters = () => {
        ...
        router.get(route('admin.logs.login'), filtersForBackend, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const resetFilters = () => {
        ...
        router.get(route('admin.logs.login'));
    };
    ...
    const exportLogs = () => {
        ...
        window.open(route('admin.logs.export', { ...filtersForExport, type: 'login' }));
    };
            ...
            <div className="py-8">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div className="flex justify-between items-center">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900">Log Login</h2>
                            <p className="text-gray-600">Monitor aktivitas login dan logout pengguna</p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold text-green-600">{stats.total_logins.toLocaleString()}</div>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold text-blue-600">{stats.today_logins.toLocaleString()}</div>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold text-red-600">{stats.failed_logins.toLocaleString()}</div>
                            </CardContent>
                        </Card>

                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-sm font-medium">Sesi Aktif Hari Ini</CardTitle>
                                <Users className="h-4 w-4 text-muted-foreground" />
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold text-purple-600">{stats.active_sessions.toLocaleString()}</div>
                            </CardContent>
                        </Card>
                    </div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
                                ...
                                <div>
                                    <Label htmlFor="aktivitas">Status Login</Label>
                                    <Select value={localFilters.aktivitas} onValueChange={(value) => handleFilterChange('aktivitas', value)}>
                                        ...
                                        <SelectContent>
                                            ...
                                            <SelectItem value="login">Login Berhasil</SelectItem>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle>Daftar Log Login</CardTitle>
                            <CardDescription>
                                Menampilkan {logs.total.toLocaleString()} aktivitas login
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <Table>
                                        ...
                                        <TableRow key={log.id}>
                                            ...
                                            <TableCell>
                                                <Badge className={`${getActivityColor(log.aktivitas)} flex items-center gap-1 w-fit`}>
                                                    ...
                                                    {log.aktivitas === 'login' ? 'Login' : 
                                                     log.aktivitas === 'logout' ? 'Logout' : 
                                                     log.aktivitas === 'login_failed' ? 'Login Gagal' : log.aktivitas}
                                                </Badge>
                                            </TableCell>
                                            <TableCell>
                                            ...
                                            <TableCell>
                                                <div className="text-sm">
                                                    <div>{getBrowserFromUserAgent(log.user_agent)}</div>
                                                    <div className="text-gray-500">{getOSFromUserAgent(log.user_agent)}</div>
                                                </div>
                                            </TableCell>
                                            <TableCell>
